...
- Calibration status is curated in `registry/calibration.json` instead, and overlaid onto the generated catalogue at the end of `proto-sync`. That is what makes a promotion durable. Absence from its `tools` map means `needs_calibration` — silence is never a promotion — and a curated key that proto-tools no longer lists is reported as an orphan rather than dropped. Promoting a tool to `validated` requires measured reliability on a held-out benchmark (framework §6) and changes what the system may claim: §6 forbids *ranking* candidates on an uncalibrated evaluator, so until a tool is promoted its scores may run but may not order anything.
