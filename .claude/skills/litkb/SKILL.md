...
`label` also sets `testable_by.rankable_by`: the subset of `tools` whose relevant metric is calibrated (`validated`), not merely present. `requires_new_evaluator: false` plus a `tools` list says only "something can *measure* this" — it does not say anything may *rank* on it. Today `rankable_by` is `[]` for every item in the repository, because nothing in `registry/calibration.json` has been promoted yet.
...
- Regenerate `registry/proto_catalog.json` with `proto-sync` rather than hand-editing it. **Every field in it is derived** — `measures` from `proto-tools output <key>`, constraints from the JSON Schema — and every `proto-sync` run overwrites the whole file. Never hand-edit it, including `status`: `build_catalog` writes `needs_calibration` for all 140 tools unconditionally, so an edit there survives only until the next sync.
- Calibration status is curated in `registry/calibration.json` instead, and overlaid onto the generated catalogue at the end of `proto-sync`. That is what makes a promotion durable. The curated unit is a **(tool, metric) pair**, not a tool: `registry/calibration.json` keys on `tools.<tool_key>.metrics.<metric_name>`, because one model can emit several metrics and be characterised for some and not others. Absence of a metric's record means `needs_calibration` for that metric — silence is never a promotion — and a curated key that proto-tools no longer lists is reported as an orphan rather than dropped. Promoting a `(tool, metric)` pair to `validated` requires a numeric `measured_error` and a `benchmark`, or the loader refuses it outright. Tool-level `status` in the catalogue is never curated directly; it is *derived* — a tool is `validated` only if it declares at least one `primary` metric and every one of them is validated. That derivation changes what the system may claim: framework §6 forbids *ranking* candidates on an uncalibrated evaluator, so until the specific metric a gate uses is validated for the tools it names, that gate's scores may run but may not order anything.
