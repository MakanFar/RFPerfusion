...
| **Relationship to `PRD-instance-tlpa.md`** | Complementary. `PRD-instance-tlpa.md` is instance #1; if the two conflict during the hackathon, `PRD-instance-tlpa.md` wins |
| **Date** | 2026-08-15 |
...
**L1.5 Falsification.** Adversarial stage, scored on routes eliminated. Three checks: physical bounds (order-of-magnitude against conservation laws), evidence status, evaluator availability. `PRD-instance-tlpa.md` §2.1 is this stage hardcoded for one instance.
...
**EvidenceItem, Candidate, ScoredCandidate:** reuse `PRD-instance-tlpa.md` §6.2 and §6.4 unchanged, plus `spec_id` and `route_id` on `ScoredCandidate`.
...
|---|---|---|---|
| **A** | SWIR transcriptional switch (`PRD-instance-tlpa.md`) | Soluble coiled-coil | Strong evaluators, known ground truth (Piraner) |
...
| Framework work cannibalizes the hackathon artifact | **High** | `PRD-instance-tlpa.md` tiebreaker stands |
